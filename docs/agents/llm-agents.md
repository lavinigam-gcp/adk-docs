...
While the `planner` parameter allows you to specify how an `LlmAgent` approaches complex tasks, the `built_in_planner` can be further configured with a `thinking_config` to enable step-by-step planning. This leverages the underlying LLM's built-in thinking capabilities, allowing it to break down problems, consider options, and execute actions sequentially.\n\nWhen `thinking_config` is enabled, the agent will attempt to generate intermediate thoughts and plans before producing a final response or executing a tool. This can significantly improve the agent's ability to handle multi-step problems and complex instructions.\n\n=== \"Python\"\n\nTo use the `built_in_planner` with thinking capabilities, instantiate `types.ThinkingConfig` with `enabled=True` and pass it to the `BuiltInPlanner` constructor. Then, provide this planner instance to your `LlmAgent`:\n\n```python\nfrom google.genai import types\nfrom adk.agents import LlmAgent\nfrom adk.planners import BuiltInPlanner\n\n# Configure thinking capabilities for the built-in planner\nthinking_config = types.ThinkingConfig(enabled=True)\n\n# Instantiate the built-in planner with thinking enabled\nstep_by_step_planner = BuiltInPlanner(thinking_config=thinking_config)\n\n# Create an LlmAgent and assign the configured planner\nplanning_agent = LlmAgent(\n    name=\"planning_agent\",\n    description=\"An agent capable of step-by-step planning and reasoning.\",\n    model=\"gemini-1.5-flash\", # Or your preferred model\n    planner=step_by_step_planner,\n    # Other agent configurations like tools, instructions, etc.\n)\n\n# You can now interact with planning_agent, and it will use its thinking capabilities.\n# For example:\n# response = await planning_agent.send_message(\"Plan a trip to Paris for 3 days, including flights and accommodation.\")\n```\n\n=== \"Java\"\n\nThe `built_in_planner` with `thinking_config` is currently available only in the Python SDK.\n