...
## Planning & Step-by-Step Execution\n\nFor `LlmAgent`s that need to tackle complex problems by breaking them down into\nmanageable steps, the `planner` parameter is essential. By integrating a\nplanner, your agent can leverage the LLM's capabilities to generate a\nstep-by-step thought process before attempting to solve a problem or use tools.\nThis significantly improves the agent's ability to reason, execute tasks, and\nrecover from errors.\n\nThe ADK provides `BuiltInPlanner` which utilizes the underlying LLM's native\nthinking capabilities. To enable this, you configure the `thinking_config`\nfield.\n\n### Configuring BuiltInPlanner with thinking_config\n\nThe `BuiltInPlanner` requires a `thinking_config` object from `google.genai.types`.\nThis configuration object tells the LLM how to engage its internal planning or\nreasoning mechanisms. The exact fields available within `types.ThinkingConfig`\ncan vary depending on the specific LLM model and `google.genai` library version.\nOften, simply instantiating `types.ThinkingConfig()` is enough to enable basic\nthinking, or it might accept parameters like `enable_reasoning=True` or\n`max_steps=N` for more granular control.\n\nHere's how to instantiate `BuiltInPlanner` and pass it to your `LlmAgent`:\n\n```python\nimport google.genai.types as types\nfrom google.adk.agents import LlmAgent\nfrom google.adk.planners import BuiltInPlanner\n\n# 1. Define the thinking configuration for the built-in planner.\n# The exact fields of types.ThinkingConfig depend on the underlying LLM.\n# For models that support built-in thinking, this typically enables or configures\n# their internal step-by-step reasoning process.\n# If no specific configuration is needed beyond enabling, an empty instance might suffice.\nthinking_config = types.ThinkingConfig()\n\n# Example with illustrative parameters (check your LLM's documentation for actual fields):\n# thinking_config = types.ThinkingConfig(enable_reasoning=True, max_thought_steps=5)\n\n# 2. Instantiate the BuiltInPlanner with the thinking configuration.\nplanner = BuiltInPlanner(thinking_config=thinking_config)\n\n# 3. Create your LlmAgent, passing the configured planner.\nplanning_agent = LlmAgent(\n    name=\"complex_task_solver\",\n    description=\"An agent designed to solve complex problems using step-by-step planning.\",\n    model=\"gemini-1.5-pro\", # Use a model that supports thinking features\n    planner=planner,\n    # Add other configurations as needed, e.g., instructions, tools\n    instructions=\"You are an expert problem solver. Always break down complex requests into a series of smaller, actionable steps before providing a final answer.\",\n    # tools=[my_tool_instance] # If the agent needs to use tools during its planning or execution\n)\n\n# Example of how the agent might be used:\n# from google.adk.channels import ConsoleChannel\n#\n# async def run_agent():\n#     channel = ConsoleChannel(planning_agent)\n#     print(\"Agent ready. Type your query:\")\n#     await channel.start()\n#\n# if __name__ == \"__main__\":\n#     import asyncio\n#     asyncio.run(run_agent())\n#\n# # When you interact with 'planning_agent', it will attempt to generate\n# # a plan based on its instructions and the thinking_config before responding.\n```\n\nBy integrating a `planner`, your `LlmAgent` gains enhanced reasoning capabilities,\nmaking it more robust and effective for tasks requiring multi-step thought processes\nor complex tool interactions.