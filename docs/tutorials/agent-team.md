...
### Enabling Step-by-Step Planning with `BuiltInPlanner`\n\nTo empower your `LlmAgent` with advanced step-by-step planning and execution capabilities, you can leverage the `planner` parameter, specifically by configuring a `BuiltInPlanner` with `thinking_config`. This allows the agent to generate a plan before attempting to solve complex problems, leading to more robust and accurate responses.\n\nHere's how to integrate a built-in planner into your `LlmAgent`:\n\n```python\nfrom google.genai import types\nfrom google.adk.planners import BuiltInPlanner\nfrom google.adk.agents import LlmAgent\nfrom google.adk.models import Gemini # Assuming Gemini is used for the LLM\n\n# Initialize your LLM model (e.g., Gemini)\nmy_llm = Gemini(model_name=\"gemini-pro\")\n\n# 1. Define the thinking configuration for the planner.\n#    Setting `enable_thinking=True` prompts the LLM to generate a detailed plan\n#    of action before proceeding with execution.\nthinking_config = types.ThinkingConfig(enable_thinking=True)\n\n# 2. Instantiate the BuiltInPlanner with your thinking configuration.\nplanner = BuiltInPlanner(thinking_config=thinking_config)\n\n# 3. Pass the configured planner to your LlmAgent.\n#    Replace `... other agent configurations ...` with your agent's specific setup,\n#    such as tools, flows, etc.\nweather_agent = LlmAgent(\n    llm=my_llm,\n    planner=planner,\n    # For example, if your agent needs to call a weather tool:\n    # tools=[weather_tool],\n    # flows=[adk.flows.llm_flows.AutoFlow()],\n)\n\nprint(\"LlmAgent initialized with built-in planning capabilities.\")\n```\n\nFor a comprehensive list of `ThinkingConfig` options and other details about planners, refer to the API reference documentation:\n*   [`LlmAgent` API Reference](api-reference/python/google-adk.html#google.adk.agents.LlmAgent)\n*   [`BuiltInPlanner` API Reference](api-reference/python/google-adk.html#google.adk.planners.BuiltInPlanner)\n*   [`ThinkingConfig` API Reference](api-reference/python/google-genai.types.html#google.genai.types.ThinkingConfig)\n